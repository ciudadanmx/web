...
const TaxisRoute = () => {
    ...
    useEffect(() => {
        if (window.google) return;  // Verificar si Google Maps ya ha sido cargado

        const script = document.createElement('script');
        script.src = `https://maps.googleapis.com/maps/api/js?key=${process.env.REACT_APP_PLACES_KEY}&libraries=places,marker&async=true`;  // Añadir async
        script.async = true;  // Asegurar que el script se cargue de manera asíncrona
        script.onload = () => {
        ...
        return () => {
            document.head.removeChild(script);  // Eliminar el script cuando el componente se desmonte
        };
    }, []);

    // Obtener la ubicación inicial usando geolocalización
    useEffect(() => {
    ...
    // Manejo de cambios en la ubicación de destino
